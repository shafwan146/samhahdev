...
@section('content')
    ...
    <div class="page-header">
        <div class="page-header-left">
            <div class="breadcrumb">
                ...
                <span>/</span>
                <a href="{{ route('admin.configs.index', ['type' => $config->type]) }}">Konfigurasi</a>
                <span>/</span>
                <span>Edit</span>
            </div>
            <h1>Edit Konfigurasi</h1>
            <p>Perbarui data {{ $config->label }}</p>
        </div>
    </div>
    ...
    <div class="card" style="max-width: 600px;">
        <div class="card-header">
            ...
            <span class="badge badge-{{ $config->is_active ? 'success' : 'gray' }}">
            ...
            <form action="{{ route('admin.configs.update', $config) }}" method="POST">
                ...
                <div class="form-group">
                    ...
                    >
                        @foreach($configTypes as $value => $label)
                            <option value="{{ $value }}" {{ old('type', $config->type) === $value ? 'selected' : '' }}>
                                {{ $label }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="alert alert-info" style="margin-top: 1rem;">
                    ...
                    <div class="alert-content">
                        <strong>Terakhir diperbarui:</strong> {{ $config->updated_at->format('d M Y, H:i') }} WIB
                    </div>
                </div>
                ...
                <div class="btn-group" style="margin-top: 1.5rem;">
                    ...
                    <a href="{{ route('admin.configs.index', ['type' => $config->type]) }}" class="btn btn-secondary btn-lg">
                        Batal
                    </a>
                </div>
            </form>
